"use client";

import { MemorialPlanningWebCapture } from "@/components/capture/memorial-planning/web-capture";
import { useCaptureScale } from "@/components/projects/mockups/use-capture-scale";

import "@/components/capture/memorial-planning/memorial-planning-capture.css";

const DESIGN_W = 1600;
const DESIGN_H = 900;

/** Live Memorial Planning portal — scaled from 1600×900 for BrowserFrame. */
export function MemorialPlanningWebMock() {
  const { hostRef, scale } = useCaptureScale(DESIGN_W, DESIGN_H);

  return (
    <div
      ref={hostRef}
      data-live-web-mockup="memorial-planning"
      className="relative aspect-[16/9] w-full overflow-hidden bg-[#F7F7F4]"
      aria-hidden="true"
    >
      <div
        className="absolute left-0 top-0 origin-top-left"
        style={{
          width: DESIGN_W,
          height: DESIGN_H,
          transform: `scale(${scale})`,
        }}
      >
        <MemorialPlanningWebCapture />
      </div>
    </div>
  );
}
