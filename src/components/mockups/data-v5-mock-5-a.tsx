...
const lineIn: Variants = {
  ...
  visible: { opacity: 1, x: 0, transition: { duration: 0.42, ease: settle } },
};
...
function Field({
  ...
  return (
    <div className="grid grid-cols-[5.5rem_1fr] items-baseline gap-2 sm:grid-cols-[6.25rem_1fr]">
      ...
      <span
        className={`truncate text-[11px] font-medium leading-snug sm:text-xs ${className ?? "text-foreground"}`}
      >
        {value}
      </span>
    </div>
...
function ManifestSheet({
  ...
  return (
    <motion.li
      className={`relative list-none rounded-sm border px-3 py-3 sm:px-3.5 sm:py-3.5 ${
        isPrimary
          ? "border-accent/25 bg-background shadow-[0_8px_20px_-12px_rgb(var(--shadow-color)/0.35)]"
    ...
    >
      <motion.div variants={lineIn}>
        ...
        <div className="mt-1.5">
          <Field label="Store" value={row.store} className="text-accent" />
      ...
      <motion.div variants={lineIn}>
        ...
        <div className="mt-1.5">
          <Field label="Retrieval" value={row.retrieval} className="text-accent-secondary" />
        </div>
      </motion.div>
...
function SolarizedClipboard({ reduced }: { reduced: boolean }) {
  return (
    <div className="relative mx-auto w-full max-w-[19.5rem] sm:max-w-[22rem]">
      ...
      <motion.div
        className="relative rounded-[1.15rem] border border-border/80 bg-surface-elevated p-[7px] shadow-[0_28px_56px_-32px_rgb(var(--shadow-color)/0.45)] sm:rounded-[1.25rem] sm:p-2"
        variants={stage}
        initial={reduced ? false : "hidden"}
        whileInView="visible"
      ...
      >
        <div
          aria-hidden="true"
          className="absolute left-1/2 top-0 z-20 h-5 w-[4.5rem] -translate-x-1/2 -translate-y-[38%] rounded-b-md border border-border/60 bg-surface sm:h-[1.375rem] sm:w-20"
        />

        <div className="overflow-hidden rounded-[0.85rem] border border-border/50 bg-surface sm:rounded-[0.95rem]">
          ...
          <ol className="space-y-2 px-2 pb-2.5 sm:space-y-2.5 sm:px-2.5 sm:pb-3">
            {manifests.map((row, index) => (
              <ManifestSheet key={row.id} row={row} index={index} reduced={reduced} />
            ))}
          </ol>
...
export function DataV5Mock5A() {
  const reduced = usePrefersReducedMotion();

  return (
    <section
      id="data-v5-mock-5-a"
      aria-labelledby="data-v5-mock-5-a-title"
      className="scroll-mt-28 px-4 py-20 sm:px-6 sm:py-28"
    >
      <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-2 lg:gap-20">
        <div className="lg:order-1" aria-hidden="true">
          <SolarizedClipboard reduced={reduced} />
        </div>
