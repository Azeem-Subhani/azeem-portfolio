"use client";

import { motion, type Variants } from "motion/react";

import { usePrefersReducedMotion } from "@/hooks/use-prefers-reduced-motion";

const settle = [0.16, 1, 0.3, 1] as const;

const stage: Variants = {
  hidden: {},
  visible: {
    transition: { staggerChildren: 0.12, delayChildren: 0.06 },
  },
};

const slipFanIn: Variants = {
  hidden: { opacity: 0, y: 24, rotate: 0, scale: 0.94 },
  visible: (custom: { rotate: number; x: number; y: number }) => ({
    opacity: 1,
    y: custom.y,
    x: custom.x,
    rotate: custom.rotate,
    scale: 1,
    transition: { duration: 0.72, ease: settle },
  }),
};

const lineIn: Variants = {
  hidden: { opacity: 0, x: -6 },
  visible: {
    opacity: 1,
    x: 0,
    transition: { duration: 0.42, ease: settle },
  },
};

const stampIn: Variants = {
  hidden: { opacity: 0, scale: 1.35, rotate: -14 },
  visible: {
    opacity: 1,
    scale: 1,
    rotate: -6,
    transition: { type: "spring", stiffness: 420, damping: 22, delay: 0.85 },
  },
};

type ManifestRow = {
  id: string;
  workload: string;
  store: string;
  storeNote: string;
  query: string;
  retrieval: string;
  rail: string;
  amount: string;
  settled: boolean;
};

const manifests: ManifestRow[] = [
  {
    id: "relational",
    workload: "Booking hold",
    store: "PostgreSQL",
    storeNote: "row locked until checkout",
    query: "slot availability",
    retrieval: "2 policy docs · 0.91",
    rail: "Stripe",
    amount: "$890.00",
    settled: true,
  },
  {
    id: "session",
    workload: "Checkout session",
    store: "DynamoDB",
    storeNote: "partition key on cart id",
    query: "installment schedule",
    retrieval: "1 excerpt · 38ms",
    rail: "Trust Commerce",
    amount: "$1,280.00",
    settled: true,
  },
  {
    id: "sync",
    workload: "Roster update",
    store: "Firestore",
    storeNote: "live sync to clients",
    query: "dues batch lookup",
    retrieval: "4 receipts matched",
    rail: "Stripe Connect",
    amount: "$340.00",
    settled: true,
  },
];

type FanSlot = {
  row: ManifestRow;
  rotate: number;
  x: number;
  y: number;
  z: number;
  dimmed: boolean;
  isPrimary: boolean;
};

/** Left and right peek from behind; center slip is fully readable. */
const fanSlots: FanSlot[] = [
  {
    row: manifests[1],
    rotate: -8,
    x: -52,
    y: 10,
    z: 10,
    dimmed: true,
    isPrimary: false,
  },
  {
    row: manifests[0],
    rotate: 0,
    x: 0,
    y: 0,
    z: 30,
    dimmed: false,
    isPrimary: true,
  },
  {
    row: manifests[2],
    rotate: 8,
    x: 52,
    y: 10,
    z: 20,
    dimmed: true,
    isPrimary: false,
  },
];

function Rule() {
  return <div aria-hidden="true" className="my-2.5 h-px bg-border sm:my-3" />;
}

function Field({
  label,
  value,
  tint,
}: {
  label: string;
  value: string;
  tint?: "accent" | "success" | "secondary";
}) {
  const valueClass =
    tint === "accent"
      ? "text-accent"
      : tint === "success"
        ? "text-success"
        : tint === "secondary"
          ? "text-accent-secondary"
          : "text-foreground";

  return (
    <div className="grid grid-cols-[5.5rem_1fr] items-baseline gap-2 sm:grid-cols-[6.25rem_1fr]">
      <span className="text-[10px] leading-tight text-muted-foreground sm:text-[11px]">
        {label}
      </span>
      <span
        className={`truncate text-[11px] font-medium leading-snug sm:text-xs ${valueClass}`}
      >
        {value}
      </span>
    </div>
  );
}

function ManifestSlip({
  slot,
  reduced,
}: {
  slot: FanSlot;
  reduced: boolean;
}) {
  const { row, rotate, x, y, z, dimmed, isPrimary } = slot;

  return (
    <motion.li
      className="absolute left-1/2 top-0 w-[13.5rem] list-none -translate-x-1/2 rounded-sm border border-border bg-background px-3 py-3 shadow-[0_1px_0_rgb(var(--shadow-color)/0.06),0_14px_32px_-18px_rgb(var(--shadow-color)/0.35)] sm:w-[14.75rem] sm:px-3.5 sm:py-3.5"
      style={{ zIndex: z, opacity: dimmed ? 0.62 : 1 }}
      custom={{ rotate, x, y }}
      variants={slipFanIn}
    >
      <motion.div variants={lineIn}>
        <Field label="Workload" value={row.workload} />
        <div className="mt-1.5">
          <Field label="Store" value={row.store} tint="accent" />
          {isPrimary && (
            <p className="mt-0.5 pl-[5.5rem] text-[9px] leading-4 text-muted-foreground sm:pl-[6.25rem] sm:text-[10px]">
              {row.storeNote}
            </p>
          )}
        </div>
      </motion.div>

      {isPrimary && (
        <>
          <Rule />

          <motion.div variants={lineIn}>
            <Field label="Query" value={row.query} />
            <div className="mt-1.5">
              <Field label="Retrieval" value={row.retrieval} tint="success" />
            </div>
          </motion.div>

          <Rule />

          <motion.div className="relative" variants={lineIn}>
            <Field label="Rail" value={row.rail} tint="secondary" />
            <div className="mt-1.5">
              <Field label="Amount" value={row.amount} />
            </div>

            {row.settled && (
              <motion.div
                className="pointer-events-none absolute bottom-1 right-2 sm:bottom-0.5 sm:right-3"
                variants={stampIn}
                initial={reduced ? false : "hidden"}
                whileInView={reduced ? undefined : "visible"}
                viewport={{ once: true, margin: "-8% 0px" }}
              >
                <div className="rounded-sm border-2 border-success px-2 py-0.5 text-[10px] font-medium tracking-wide text-success opacity-90 sm:text-[11px]">
                  Settled
                </div>
              </motion.div>
            )}
          </motion.div>
        </>
      )}

      {!isPrimary && (
        <div className="mt-2 border-t border-dashed border-border/80 pt-2">
          <p className="text-[9px] leading-4 text-muted-foreground sm:text-[10px]">
            {row.rail} · {row.amount}
          </p>
        </div>
      )}
    </motion.li>
  );
}

function SettlementFan({ reduced }: { reduced: boolean }) {
  return (
    <div className="relative mx-auto w-full max-w-[20rem] sm:max-w-[23rem]">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute left-1/2 top-[44%] h-48 w-52 -translate-x-1/2 -translate-y-1/2 rounded-full bg-accent/14 blur-3xl"
      />

      <motion.div
        className="relative overflow-hidden rounded-[1.15rem] border border-border bg-surface shadow-[0_28px_56px_-32px_rgb(var(--shadow-color)/0.4)] sm:rounded-[1.25rem]"
        variants={stage}
        initial={reduced ? false : "hidden"}
        whileInView="visible"
        viewport={{ once: true, margin: "-10% 0px" }}
      >
        <div className="flex items-end justify-between gap-3 border-b border-border bg-surface-elevated/70 px-3.5 py-3 sm:px-4 sm:py-3.5">
          <div>
            <p className="font-display text-[1.05rem] leading-tight text-foreground sm:text-lg">
              Store routing
            </p>
            <p className="mt-0.5 text-[10px] leading-4 text-muted-foreground sm:text-[11px]">
              triplicate manifest
            </p>
          </div>
          <p className="text-right text-[10px] leading-4 text-muted-foreground sm:text-[11px]">
            RAG at
            <br />
            query time
          </p>
        </div>

        <div className="relative h-[15.5rem] px-3 pt-5 sm:h-[16.5rem] sm:px-4 sm:pt-6">
          <ol className="relative m-0 h-full p-0">
            {fanSlots.map((slot) => (
              <ManifestSlip key={slot.row.id} slot={slot} reduced={reduced} />
            ))}
          </ol>
        </div>

        <div className="border-t border-border bg-surface-elevated/50 px-3.5 py-2.5 sm:px-4">
          <p className="text-[10px] leading-4 text-muted-foreground sm:text-[11px]">
            Each row posts to the store that owns it. Stripe and Trust Commerce capture
            against that write — not a separate ledger hunt.
          </p>
        </div>
      </motion.div>
    </div>
  );
}

export function DataV5Mock5H() {
  const reduced = usePrefersReducedMotion();

  return (
    <section
      id="data-v5-mock-5-h"
      aria-labelledby="data-v5-mock-5-h-title"
      className="scroll-mt-28 px-4 py-20 sm:px-6 sm:py-28"
    >
      <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-2 lg:gap-20">
        <div className="lg:order-1" aria-hidden="true">
          <SettlementFan reduced={reduced} />
        </div>

        <div className="lg:order-2">
          <motion.div
            initial={reduced ? false : { opacity: 0 }}
            whileInView={reduced ? undefined : { opacity: 1 }}
            viewport={{ once: true, margin: "-10% 0px" }}
            transition={{ duration: 0.55, ease: settle }}
          >
            <h2
              id="data-v5-mock-5-h-title"
              className="font-display text-[clamp(2rem,5vw,3.75rem)] font-normal leading-[1.05] tracking-tight"
            >
              Enterprise data management
            </h2>
            <p className="mt-5 max-w-md text-base leading-7 text-muted-foreground sm:text-lg">
              Postgres holds relational records. DynamoDB and Firestore take over when
              the product needs partition keys or live sync. RAG retrieval answers from
              those stores in one pass.
            </p>
            <p className="mt-4 max-w-md text-sm leading-6 text-muted-foreground">
              Stripe and Trust Commerce settle against whichever store posted the row. I
              wire that path at schema time so charges never chase the wrong home.
            </p>
          </motion.div>
        </div>
      </div>
    </section>
  );
}
