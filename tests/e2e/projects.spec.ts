...
test.describe("projects catalog", () => {
  test("renders the full catalog before any filter is applied", async ({ page }) => {
    ...
    await expect(page.locator("[data-project-slug]")).toHaveCount(projects.length);
  });
  ...
  test("filtering narrows the grid and announces the new count", async ({ page }) => {
    ...
    await expect(page.locator("[data-project-slug]")).toHaveCount(paymentsCount);
  ...
  test("reduced motion skips the entrance animation entirely", async ({ page }) => {
    ...
    const firstProject = page.locator("[data-project-slug]").first();
    await expect(firstProject).toBeVisible();
    await expect(page.locator(".catalog-stage-visual")).toBeVisible();
  });
});
